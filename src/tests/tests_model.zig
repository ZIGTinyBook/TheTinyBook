...
test "Model with multiple layers forward test" {
    ...
    try model.addLayer(LayerTypes.DenseLayer, 3, 2, ActivationType.ReLU);
    ...
    try model.addLayer(LayerTypes.DenseLayer, 2, 3, ActivationType.ReLU);

    std.debug.print("\n>>>>>> model info: n_layers :{} ", .{model.layers.len});
